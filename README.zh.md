...
> - `stig_compliance_check.sh`：修复 `printf %d` 接收 `grep -c || echo 0` 双输出导致的格式错误（严重度分级输出 invalid number）
> - `cis_benchmark_audit.sh`：修复 JSON 报告因 `evidence` 和 `desc` 字段未转义控制字符（换行/制表/回车）导致的 JSON 无效问题
...
| [09](handbook/09-cis-stig-compliance.md) | CIS 与 STIG 合规 | 自动化 CIS Benchmark 和 DISA STIG 合规审计 |
